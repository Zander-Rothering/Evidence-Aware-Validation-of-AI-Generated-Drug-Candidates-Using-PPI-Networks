...
if __name__ == '__main__':
        ...
        if prop_condition is None and scaf_condition is None:
            ...
            for i in tqdm(range(gen_iter)):
                    x = torch.tensor([stoi[s] for s in regex.findall(context)], dtype=torch.long)[None,...].repeat(args.batch_size, 1).to('mps')
                    p = None
                    # p = torch.tensor([[c]]).repeat(args.batch_size, 1).to('mps')   # for single condition
                    # p = torch.tensor([c]).repeat(args.batch_size, 1).unsqueeze(1).to('mps')    # for multiple conditions
                    #sca = torch.tensor([stoi[s] for s in regex.findall(j)], dtype=torch.long)[None,...].repeat(args.batch_size, 1).to('mps')
                    sca = None
